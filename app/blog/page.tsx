...
export const metadata: Metadata = {
  ...
  description:
    'Data-backed strategies for reducing SaaS churn, recovering failed payments, and building retention systems that run on autopilot. Written by the team building ChurnGuard.',
  openGraph: {
    title: 'Blog — ChurnGuard | SaaS Churn Prevention Insights',
    description:
...
const TAG_COLORS: Record<string, string> = {
  'churn prevention': '#312e81',
  'customer retention': '#1e3a5f',
  'SaaS metrics': '#1c2e1c',
  'payment recovery': '#2d1f1f',
  dunning: '#2d1f1f',
  'SaaS revenue': '#1c2e1c',
  'churn signals': '#312e81',
  'customer health': '#1c2e1c',
  'early warning': '#2d1f1f',
};

const TAG_TEXT: Record<string, string> = {
  'churn prevention': '#a5b4fc',
  'customer retention': '#7dd3fc',
  'SaaS metrics': '#6ee7b7',
  'payment recovery': '#fca5a5',
  dunning: '#fca5a5',
  'SaaS revenue': '#6ee7b7',
  'churn signals': '#a5b4fc',
  'customer health': '#6ee7b7',
  'early warning': '#fca5a5',
};
    ...
    <div style={{ minHeight: '100vh', background: '#0f172a', color: '#f1f5f9', fontFamily: 'system-ui,-apple-system,sans-serif' }}>
      ...
      >
        <nav style={{ maxWidth: '1152px', margin: '0 auto', padding: '0 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '64px' }}>
          <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: '8px', fontWeight: '700', fontSize: '18px', color: '#f1f5f9', textDecoration: 'none' }}>
            <div style={{ width: '32px', height: '32px', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '14px', background: 'linear-gradient(135deg,#6366f1,#8b5cf6)' }}>
              🛡️
            </div>
            ChurnGuard
          </Link>
          <div style={{ display: 'flex', alignItems: 'center', gap: '24px' }}>
            <Link href="/#features" style={{ color: '#94a3b8', fontSize: '14px', textDecoration: 'none' }}>Features</Link>
            <Link href="/#pricing" style={{ color: '#94a3b8', fontSize: '14px', textDecoration: 'none' }}>Pricing</Link>
      ...
      <div style={{ maxWidth: '1152px', margin: '0 auto', padding: '80px 24px 48px' }}>
        ...
        {featured && (
          <Link href={`/blog/${featured.slug}`} style={{ display: 'block', textDecoration: 'none', marginBottom: '48px' }}>
            <div style={{
              ...
              onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.borderColor = 'rgba(99,102,241,0.5)'; (e.currentTarget as HTMLDivElement).style.transform = 'translateY(-2px)'; }}
              onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.borderColor = 'rgba(99,102,241,0.2)'; (e.currentTarget as HTMLDivElement).style.transform = 'none'; }}
            >
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px', flexWrap: 'wrap' }}>
                ...
                <span style={{ marginLeft: 'auto', fontSize: '14px', color: '#818cf8', fontWeight: '600' }}>
                  Read article →
                </span>
              </div>
            </div>
          ...
          {rest.map(post => (
            <Link key={post.slug} href={`/blog/${post.slug}`} style={{ textDecoration: 'none' }}>
              <article
                ...
                onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = '#334155'; (e.currentTarget as HTMLElement).style.transform = 'none'; }}
              >
                {/* Tags */}
                <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginBottom: '14px' }}>
                  {post.tags.slice(0, 2).map(tag => (
                    <span key={tag} style={{
                      ...
                      background: TAG_COLORS[tag] ?? '#1f2937',
                      color: TAG_TEXT[tag] ?? '#9ca3af',
        ...
        <div style={{ marginTop: '80px', textAlign: 'center', padding: '56px 32px', background: 'linear-gradient(135deg,rgba(99,102,241,0.06),rgba(139,92,246,0.04))', border: '1px solid rgba(99,102,241,0.15)', borderRadius: '24px' }}>
          ...
          <p style={{ color: '#94a3b8', fontSize: '16px', margin: '0 0 28px', maxWidth: '460px', marginLeft: 'auto', marginRight: 'auto', lineHeight: '1.6' }}>
      ...
      {/* ── Footer ── */}
      <footer style={{ borderTop: '1px solid #1e293b', padding: '32px 24px', textAlign: 'center', color: '#64748b', fontSize: '13px', marginTop: '80px' }}>
        <p style={{ margin: 0 }}>© 2026 ChurnGuard. All rights reserved. · <Link href="/blog" style={{ color: '#64748b' }}>Blog</Link> · <Link href="/#pricing" style={{ color: '#64748b' }}>Pricing</Link></p>
      </footer>
    </div>
  );
}
