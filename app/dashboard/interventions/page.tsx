...
export default function InterventionsPage() {
  ...
  async function fetchInterventions() {
    try {
      ...
      if (data.interventions) {
        ...
        updateStats(data.interventions);
      }
    } catch (error) {
  ...
  function updateStats(interventionsList: Intervention[]) {
    const pending = interventionsList.filter((i) => i.status === 'pending').length;
    const saved = interventionsList.filter((i) => i.status === 'saved').length;
    const churned = interventionsList.filter((i) => i.status === 'churned').length;
    setStats({
  ...
  async function updateStatus(id: string, status: 'saved' | 'churned') {
    setUpdating(id);
    try {
      const res = await fetch('/api/interventions', {
        method: 'PATCH',
        ...
        body: JSON.stringify({
          interventionId: id,
          status: status,
          mrrSaved: status === 'saved' ? 100 : 0,
          mrrLost: status === 'churned' ? 100 : 0
        })
      });
      ...
      if (res.ok) {
        ...
        updateStats(updatedInterventions);
      } else {
  ...
  async function deleteIntervention(id: string) {
    ...
    try {
      ...
      if (res.ok) {
        const updatedList = interventions.filter(i => i.id !== id);
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: '2rem' }}>
          ...
          <p style={{ color: '#94a3b8', fontSize: '1rem' }}>
            Track and manage customer retention interventions
          </p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.5rem', marginBottom: '2rem' }}>
          <div style={{ background: '#1e293b', padding: '1.5rem', borderRadius: '0.75rem', border: '1px solid #334155' }}>
            <div style={{ color: '#94a3b8', fontSize: '0.875rem', marginBottom: '0.5rem' }}>Total</div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: '#0f172a' }}>
                  <th style={{ padding: '1rem 1.5rem', textAlign: 'left', color: '#94a3b8', fontSize: '0.75rem', fontWeight: '600', textTransform: 'uppercase' }}>Customer</th>
                  ...
                  <th style={{ padding: '1rem 1.5rem', textAlign: 'left', color: '#94a3b8', fontSize: '0.75rem', fontWeight: '600', textTransform: 'uppercase' }}>Risk</th>
                  ...
                  return (
                    <tr key={intervention.id} style={{ borderTop: '1px solid #334155' }}>
                      <td style={{ padding: '1.25rem 1.5rem', color: 'white', fontWeight: '500' }}>
                        {intervention.customerId.substring(0, 15)}...
                      </td>
                      ...
                      <td style={{ padding: '1.25rem 1.5rem' }}>
                        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                          {isPending && (
                            <>
                              <button
                                onClick={() => updateStatus(intervention.id, 'saved')}
                                ...
                                style={{
                                  ...
                                  padding: '0.5rem 1rem',
                                  borderRadius: '0.5rem',
                                  cursor: updating === intervention.id ? 'not-allowed' : 'pointer',
                                  fontSize: '0.875rem',
                                  opacity: updating === intervention.id ? 0.5 : 1
                                }}
                              >
                                {updating === intervention.id ? '⏳' : '✅ Mark Saved'}
                              </button>
                              <button
                                onClick={() => updateStatus(intervention.id, 'churned')}
                                disabled={updating === intervention.id}
                                style={{
                                  background: '#ef4444',
                                  color: 'white',
                              ...
                              >
                                {updating === intervention.id ? '⏳' : '❌ Mark Churned'}
                              </button>
                            </>
                          )}
                          <button
