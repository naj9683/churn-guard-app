...
export default function LoginPage() {
  ...
  // If already signed in when the page loads, route immediately
  useEffect(() => {
    if (!userLoaded || !user || redirected.current) return;
  ...
  async function routeAfterAuth() {
    try {
      ...
      const { hasAccess } = res.ok ? await res.json() : { hasAccess: true }; // fail open
      router.replace(hasAccess ? '/dashboard' : '/pricing');
    } catch {
      router.replace('/dashboard'); // fail open
    }
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      // Admin: passwordless ticket login
      if (email.trim().toLowerCase() === ADMIN_EMAIL.toLowerCase()) {
        ...
        if (!res.ok) throw new Error((await res.json().catch(() => ({}))).error ?? 'Admin login failed');
  ...
  if (!userLoaded || (loading && redirected.current)) {
    return (
      <div style={{
        ...
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{ color: '#9ca3af', fontSize: '14px', fontFamily: 'Inter, sans-serif' }}>Loading…</p>
      ...
      <div style={{ width: '100%', maxWidth: '420px' }}>
        ...
        <div style={{
            ...
            {!isAdminEmail && (
              <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                  ...
                  <Link href="/sign-in" style={{ fontSize: '13px', color: '#6366f1', textDecoration: 'none' }}>Forgot password?</Link>
                </div>
                <div style={{ position: 'relative' }}>
                  <input
                    ...
                    style={{ width: '100%', padding: '10px 42px 10px 14px', border: '1px solid #d1d5db', borderRadius: '8px', fontSize: '14px', outline: 'none', boxSizing: 'border-box', color: '#111827', background: '#fff' }}
                    onFocus={e => { e.target.style.borderColor = '#6366f1'; e.target.style.boxShadow = '0 0 0 3px rgba(99,102,241,0.12)'; }}
                    onBlur={e => { e.target.style.borderColor = '#d1d5db'; e.target.style.boxShadow = 'none'; }}
                  />
          ...
          <div style={{ marginTop: '24px', paddingTop: '20px', borderTop: '1px solid #f3f4f6', textAlign: 'center', fontSize: '13px', color: '#9ca3af' }}>
            ...
            <Link href="/pricing" style={{ color: '#6366f1', textDecoration: 'none', fontWeight: '500' }}>View plans →</Link>
          </div>
        </div>
